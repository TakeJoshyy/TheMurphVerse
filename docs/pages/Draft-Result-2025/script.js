...
const competitors = [
  { id: 'comp1', name: 'ROFLMAOGAMERS1', bless:'bless7', bane:`bane3` },
  { id: 'comp2', name: 'BENNYTOTS', bless:'bless3', bane:`bane6` },
  { id: 'comp3', name: 'CAPTDANIELDORE', bless:'bless2', bane:`bane2` },
  { id: 'comp4', name: 'REILUR', bless:'bless1', bane:`bane8` },
  { id: 'comp5', name: 'BEMIDGEGAMING', bless:'bless8', bane:`bane4` },
  { id: 'comp6', name: 'SAMUWELL', bless:'bless4', bane:`bane5` },
  { id: 'comp7', name: 'BATCAM22', bless:'bless9', bane:`bane7` },
  { id: 'comp8', name: 'CEEDSY', bless:'bless6', bane:`bane1` },
];
...
// Populate competitor table with prefilled blessings and banes
const tableBody = document.getElementById('competitor-table-body');
tableBody.innerHTML = competitors
  .map((comp) => {
    // Find the blessing and bane data for this competitor
    const blessData = blessings.find((b) => b.id === comp.bless);
    const baneData = banes.find((b) => b.id === comp.bane);

    // Prefill the drop zones with the corresponding card HTML if present
    const blessCell = `<td class="drop-zone" data-type="blessing">${
      blessData ? createCard(blessData) : ''
    }</td>`;
    const baneCell = `<td class="drop-zone" data-type="bane">${
      baneData ? createCard(baneData) : ''
    }</td>`;
